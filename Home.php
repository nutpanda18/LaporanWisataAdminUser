...
if ($response) {
    ...
    if (isset($data['data']['tbody'])) {
        $wisata_data = $data['data']['tbody'];
    }
}
// -------------------------------------------------
...
<head>
    ...
    <title>Sistem Laporan Wisata - Kota Madiun</title>
    ...
    <div class="container mx-auto px-4 py-10 max-w-6xl">
        ...
        <?php if ($userRole === 'admin'): ?>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 w-full">
                <div class="bg-white p-4 rounded-2xl shadow-sm border-l-4 border-orange-950 flex flex-col justify-center min-h-[100px]">
                ...
                <div class="bg-white p-4 rounded-2xl shadow-sm border-l-4 border-amber-500 flex flex-col justify-center min-h-[100px]">
                ...
                <div class="bg-white p-4 rounded-2xl shadow-sm border-l-4 border-green-600 flex flex-col justify-center min-h-[100px]">
                ...
                <?php elseif ($isLoggedIn): ?>
                    <div class="bg-white p-5 rounded-3xl shadow-xl border-t-8 border-orange-600">
                        ...
                        <form action="proses_simpan.php" method="POST" class="space-y-3">
                            ...
                            <div>
                                <label class="text-[10px] font-bold text-gray-400 uppercase">Lokasi Wisata (Data BPS)</label>
                                <select name="lokasi_wisata" class="w-full p-3 bg-stone-50 border rounded-xl text-sm outline-none focus:border-orange-500" required>
                                    <option value="">-- Pilih Lokasi --</option>
                                    <?php 
                                    if (!empty($wisata_data)) {
                                        foreach ($wisata_data as $row) {
                                            // Pada API Statictable BPS, index 1 biasanya berisi nama lokasi
                                            $nama_lokasi = $row[1] ?? null;
                                            // Filter: Jangan tampilkan jika isinya angka saja, tanda strip, atau kosong
                            ...
                            <button type="submit" class="w-full bg-orange-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-orange-700 transition">Kirim Laporan</button>
                        </form>
                    </div>
                <?php endif; ?>
            </div>
